...
public:
  ...
  static void update(real_type time, real_type dt, const real_type* state, const shared_state& shared, internal_state& internal, rng_state_type& rng_state, real_type* state_next) {
    ...
    const real_type FOI_sylv_cur = monty::math::min(shared.FOI_max, shared.FOI_spillover[t_pt - 1] * shared.time_inc);
    for (size_t i = 1; i <= static_cast<size_t>(shared.N_age); ++i) {
    ...
    const real_type FOI_urb_cur = monty::math::min(shared.FOI_max, beta * ((dust2::array::sum<real_type>(I_sylv, shared.dim.I_sylv) + dust2::array::sum<real_type>(I_urb, shared.dim.I_urb)) / P_tot));
    for (size_t i = 1; i <= static_cast<size_t>(shared.N_age); ++i) {
