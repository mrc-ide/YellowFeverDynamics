...
[[cpp11::register]]
SEXP dust_cpu_InfectionSplitModelOD_alloc(cpp11::list r_pars, bool pars_multi, size_t step,
...
[[cpp11::register]]
SEXP dust_cpu_InfectionSplitModelOD_filter(SEXP ptr, SEXP step_end,
...
public:
  ...
  void update(size_t step, const real_type * state, rng_state_type& rng_state, real_type * state_next) {
    ...
    for (int i = 1; i <= shared->N_age; ++i) {
      state_next[shared->offset_variable_E_sylvatic + i - 1] = std::max(shared->Pmin, E_sylvatic[i - 1] + internal.E_new_sylvatic[i - 1] - internal.I_new_sylvatic[i - 1]);
    }
    for (int i = 1; i <= shared->N_age; ++i) {
      state_next[shared->offset_variable_I_sylvatic + i - 1] = std::max(shared->Pmin, I_sylvatic[i - 1] + internal.I_new_sylvatic[i - 1] - internal.R_new_sylvatic[i - 1]);
    }
    for (int i = 1; i <= shared->N_age; ++i) {
      state_next[shared->offset_variable_I_urban + i - 1] = std::max(shared->Pmin, I_urban[i - 1] + internal.I_new_urban[i - 1] - internal.R_new_urban[i - 1]);
    }
    for (int i = 1; i <= shared->N_age; ++i) {
    ...
    real_type FOI_urban = std::min(shared->FOI_max, shared->beta * ((odin_sum1<real_type>(I_sylvatic, 0, shared->dim_I_sylvatic) + odin_sum1<real_type>(I_urban, 0, shared->dim_I_urban)) / (real_type) P_tot));
    {
       int i = 1;
       state_next[shared->offset_variable_R + i - 1] = std::max(shared->Pmin, R[0] + internal.R_new_sylvatic[0] + internal.R_new_urban[0] - internal.vacc_rate[0] * R[0] * internal.inv_P_nV[0] - (internal.dP2[0] * R[0] * internal.inv_P[0]));
    }
    for (int i = 2; i <= shared->N_age; ++i) {
      state_next[shared->offset_variable_R + i - 1] = std::max(shared->Pmin, R[i - 1] + internal.R_new_sylvatic[i - 1] + internal.R_new_urban[i - 1] - internal.vacc_rate[i - 1] * R[i - 1] * internal.inv_P_nV[i - 1] + (internal.dP1[i - 1] * R[i - 1 - 1] * internal.inv_P[i - 1 - 1]) - (internal.dP2[i - 1] * R[i - 1] * internal.inv_P[i - 1]));
    }
    {
       int i = 1;
    ...
    for (int i = 1; i <= shared->N_age; ++i) {
      state_next[shared->offset_variable_E_urban + i - 1] = std::max(shared->Pmin, E_urban[i - 1] + internal.E_new_urban[i - 1] - internal.I_new_urban[i - 1]);
    }
    {
       int i = 1;
       state_next[5 + i - 1] = std::max(shared->Pmin, S[0] - internal.E_new_sylvatic[0] - internal.E_new_urban[0] - internal.vacc_rate[0] * S[0] * internal.inv_P_nV[0] + internal.dP1[0] - (internal.dP2[0] * S[0] * internal.inv_P[0]));
    }
    for (int i = 2; i <= shared->N_age; ++i) {
      state_next[5 + i - 1] = std::max(shared->Pmin, S[i - 1] - internal.E_new_sylvatic[i - 1] - internal.E_new_urban[i - 1] - internal.vacc_rate[i - 1] * S[i - 1] * internal.inv_P_nV[i - 1] + (internal.dP1[i - 1] * S[i - 1 - 1] * internal.inv_P[i - 1 - 1]) - (internal.dP2[i - 1] * S[i - 1] * internal.inv_P[i - 1]));
    }
  }
private:
...
dust::pars_type<InfectionSplitModelOD> dust_pars<InfectionSplitModelOD>(cpp11::list user) {
  ...
  shared->FOI_sylvatic = std::min(shared->FOI_max, shared->FOI_spillover * shared->dt);
...
cpp11::sexp dust_info<InfectionSplitModelOD>(const dust::pars_type<InfectionSplitModelOD>& pars) {
  const InfectionSplitModelOD::internal_type internal = pars.internal;
...
SEXP dust_cpu_InfectionSplitModelOD_alloc(cpp11::list r_pars, bool pars_multi, size_t step,
                             ...
                             cpp11::sexp r_seed, bool deterministic,
                             cpp11::sexp gpu_config) {
  return dust::r::dust_cpu_alloc<InfectionSplitModelOD>(r_pars, pars_multi, step, r_n_particles,
