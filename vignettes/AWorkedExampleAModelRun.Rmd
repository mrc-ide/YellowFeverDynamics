...
For applications where only the number of new infectious people at each time point is desired, we use the case_data_generate() function instead. This functions similarly to Basic_Model_Run() and Full_Model_Run() but only outputs data on the total number (across all age groups) of newly infectious people at each time point. It is also designed to run large numbers of repetitions, automatically sequentially running smaller groups of particles and threads to avoid causing errors or freezing the computer while running (Basic_Model_Run() and Full_Model_Run() are limited to 20 particles/threads for this reason). The ability to run a large number of repetitions is valuable when modelling annual case rates and/or outbreak occurrence (see [Guide 4 - Processing Model Results](https://mrc-ide.github.io/YellowFeverDynamics/articles/BGuideDModelResultsProcess.html)) in situations where the overall force of infection is low and therefore new infections are rare. Here we run case_data_generate() for 1000 repetitions with a low spillover FOI and no human-to-human transmission, which gives a very low daily infection rate.
