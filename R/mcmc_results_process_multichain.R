...
#' @param datasets List containing one or more datasets, generated by get_mcmc_datasets_multichain()
#' @param datasets_selected Vector of dataset numbers to be selected from list
#' @param lines_selected TBA
#' @param type TBA
#' @param enviro_data TBA
#'
#' @export
#'
get_mcmc_FOI_R0_multichain <- function(datasets=c(),datasets_selected=c(1),lines_selected=list(),type="FOI+R0",
                                       enviro_data=NULL){
  #TODO Add assert_that functions
  assert_that(length(datasets_selected)==length(lines_selected))

  outputs=list()
  for(i in 1:length(datasets_selected)){
    n_data=datasets_selected[i]
    outputs[[n_data]]=get_mcmc_FOI_R0_data(datasets[[n_data]][lines_selected[[i]],],type,enviro_data)
  }
