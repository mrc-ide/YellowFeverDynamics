...
[[cpp11::register]]
SEXP dust_cpu_FullModelOD2_alloc(cpp11::list r_pars, bool pars_multi, size_t step,
...
[[cpp11::register]]
SEXP dust_cpu_FullModelOD2_update_state(SEXP ptr, SEXP r_pars, SEXP r_state,
...
[[cpp11::register]]
SEXP dust_cpu_FullModelOD2_filter(SEXP ptr, SEXP step_end,
...
public:
  ...
  FullModelOD2(const dust::pars_type<FullModelOD2>& pars) :
  ...
  void update(size_t step, const real_type * state, rng_state_type& rng_state, real_type * state_next) {
    ...
    for (int i = 1; i <= shared->N_age; ++i) {
      internal.I_new[i - 1] = dust::random::binomial<real_type>(rng_state, static_cast<int>(E[i - 1]), shared->rate1);
    }
    for (int i = 1; i <= shared->N_age; ++i) {
...
namespace dust {
template<>
dust::pars_type<FullModelOD2> dust_pars<FullModelOD2>(cpp11::list user) {
  using real_type = typename FullModelOD2::real_type;
  auto shared = std::make_shared<FullModelOD2::shared_type>();
  FullModelOD2::internal_type internal;
  shared->FOI_max = 1;
  ...
  return dust::pars_type<FullModelOD2>(shared, internal);
}
template <>
cpp11::sexp dust_info<FullModelOD2>(const dust::pars_type<FullModelOD2>& pars) {
  const FullModelOD2::internal_type internal = pars.internal;
  const std::shared_ptr<const FullModelOD2::shared_type> shared = pars.shared;
...
cpp11::sexp dust_FullModelOD2_capabilities() {
  return dust::r::dust_capabilities<FullModelOD2>();
}

cpp11::sexp dust_FullModelOD2_gpu_info() {
  return dust::gpu::r::gpu_info();
}
using model_cpu = dust::dust_cpu<FullModelOD2>;

SEXP dust_cpu_FullModelOD2_alloc(cpp11::list r_pars, bool pars_multi, size_t step,
                             cpp11::sexp r_n_particles, size_t n_threads,
                             cpp11::sexp r_seed, bool deterministic,
                             cpp11::sexp gpu_config) {
  return dust::r::dust_cpu_alloc<FullModelOD2>(r_pars, pars_multi, step, r_n_particles,
...
SEXP dust_cpu_FullModelOD2_set_data(SEXP ptr, cpp11::list data,
                                       bool shared) {
