...
public:
  ...
  void update(size_t step, const real_type * state, rng_state_type& rng_state, real_type * state_next) {
    ...
    {
       int i = 1;
       state_next[4 + i - 1] = std::max(shared->Pmin, S[0] - internal.E_new[0] - internal.V_S[0] + internal.dP1[0] - (internal.dP2[0] * internal.F_S[0]));
    }
    for (int i = 2; i <= shared->N_age; ++i) {
      state_next[4 + i - 1] = std::max(shared->Pmin, S[i - 1] - internal.E_new[i - 1] - internal.V_S[i - 1] + (internal.dP1[i - 1] * internal.F_S[i - 1 - 1]) - (internal.dP2[i - 1] * internal.F_S[i - 1]));
    }
  }
private:
