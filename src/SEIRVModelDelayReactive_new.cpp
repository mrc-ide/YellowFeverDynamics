...
public:
  ...
  static void update(real_type time, real_type dt, const real_type* state, const shared_state& shared, internal_state& internal, rng_state_type& rng_state, real_type* state_next) {
    ...
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        internal.vacc_rate_cam[i - 1 + (j - 1) * shared.dim.vacc_rate_cam.mult[1]] = flag3[i - 1] * shared.vacc_cov_cam[i - 1 + (j - 1) * shared.dim.vacc_cov_cam.mult[1]] * shared.inv_t_cam * (1 - (V[i - 1 + (j - 1) * shared.dim.V.mult[1]] * internal.inv_P[i - 1 + (j - 1) * shared.dim.inv_P.mult[1]]));
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
    ...
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        internal.vacc_rate[i - 1 + (j - 1) * shared.dim.vacc_rate.mult[1]] = (shared.vacc_rate_daily[i - 1 + (j - 1) * shared.dim.vacc_rate_daily.mult[1] + (year_i - 1) * shared.dim.vacc_rate_daily.mult[2]] + (flag3[i - 1] * internal.vacc_rate_cam[i - 1 + (j - 1) * shared.dim.vacc_rate_cam.mult[1]] * monty::math::ceil<real_type>(1 - flag4[i - 1]))) * shared.vaccine_efficacy * shared.time_inc * internal.P[i - 1 + (j - 1) * shared.dim.P.mult[1]];
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
