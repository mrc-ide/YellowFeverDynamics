...
param_dist <- read.csv(file = paste(path.package("YellowFeverDynamics"), "/exdata/enviro_coeffs_example_multi.csv", 
                                    sep = ""), header = TRUE)  # Data frame of one or more sets of parameter values
...
R0_fixed_values <- NULL # Fixed R0 values, vaccine efficacy and severe and fatal case reporting probabilities; here set 
                        # to NULL because R0, vaccine efficacy and reporting probabilities are variable parameters being
                        # estimated
...
flag_reporting=FALSE # Flag indicating whether to calculate numbers of reported cases and deaths (using p_rep_severe0 
                     # and p_rep_death0) in addition to actual numbers
...
input_data_novacc$vacc_data[,input_data_novacc$years_labels>=years_data[1],] <- 
  ...
  # Amend input data to remove all vaccination in or after the first year of the period considered
