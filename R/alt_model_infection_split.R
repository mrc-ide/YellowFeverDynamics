#-------------------------------------------------------------------------------
#' @title Infection_Split_Model_Run
#'
...
Infection_Split_Model_Run <- function(FOI_spillover=0.0,R0=1.0,vacc_data=list(),pop_data=list(),year0=1940,mode_start=0,
  ...
  return(list(day=array(x_res[2,,],dim=c(n_particles,t_pts_out)), year=array(x_res[3,,],dim=c(n_particles,t_pts_out)),
              ...
              E_sylvatic=array(x_res[c((N_age+1+n_nv):((2*N_age)+n_nv)),,],dim=c(N_age,n_particles,t_pts_out)),
              E_urban=array(x_res[c(((2*N_age)+1+n_nv):((3*N_age)+n_nv)),,],dim=c(N_age,n_particles,t_pts_out)),
              I_sylvatic=array(x_res[c(((3*N_age)+1+n_nv):((4*N_age)+n_nv)),,],dim=c(N_age,n_particles,t_pts_out)),
              I_urban=array(x_res[c(((4*N_age)+1+n_nv):((5*N_age)+n_nv)),,],dim=c(N_age,n_particles,t_pts_out)),
              R=array(x_res[c(((5*N_age)+1+n_nv):((6*N_age)+n_nv)),,],dim=c(N_age,n_particles,t_pts_out)),
              V=array(x_res[c(((6*N_age)+1+n_nv):((7*N_age)+n_nv)),,],dim=c(N_age,n_particles,t_pts_out)),
              C_sylvatic=array(x_res[c(((7*N_age)+1+n_nv):((8*N_age)+n_nv)),,],dim=c(N_age,n_particles,t_pts_out)),
              C_urban=array(x_res[c(((8*N_age)+1+n_nv):((9*N_age)+n_nv)),,],dim=c(N_age,n_particles,t_pts_out))))
}
