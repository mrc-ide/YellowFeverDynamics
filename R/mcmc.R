...
#'   vaccine_efficacy->p_rep_severe->p_rep_death. If these parameters are to be varied, the values separately supplied
...
MCMC <- function(log_params_ini=c(),input_data=list(),obs_sero_data=NULL,obs_case_data=NULL,obs_outbreak_data=NULL,
                 filename_prefix="Chain",Niter=1,type=NULL,log_params_min=c(),log_params_max=c(),n_reps=1,mode_start=0,
  ...
  const_list=list(type=type,log_params_min=log_params_min,log_params_max=log_params_max,n_reps=n_reps,
  ...
  out = MCMC_step(log_params=log_params_ini,input_data,obs_sero_data,obs_case_data,obs_outbreak_data,
  ...
  for (iter in 1:Niter){
    ...
    out = MCMC_step(log_params,input_data,obs_sero_data,obs_case_data,obs_outbreak_data,chain_cov,adapt,like_current,
                     const_list)
  }
...
#'   n_reps,mode_start,prior_type,dt=dt,enviro_data,R0_fixed_values,vaccine_efficacy,p_rep_severe,p_rep_death,
#'   m_FOI_Brazil)
#'
#' @export
#'
MCMC_step <- function(log_params=c(),input_data=list(),obs_sero_data=NULL,obs_case_data=NULL,obs_outbreak_data=NULL,
  ...
  like_prop=single_like_calc(log_params_prop,input_data,obs_sero_data,obs_case_data,obs_outbreak_data,const_list)
...
single_like_calc <- function(log_params_prop=c(),input_data=list(),obs_sero_data=NULL,obs_case_data=NULL,
  ...
  if(is.numeric(const_list$p_rep_severe)==FALSE){
    ...
    if(p_rep_severe<exp(const_list$log_params_min[names(const_list$log_params_min)=="p_rep_severe"])){prior_report=-Inf}
    if(p_rep_severe>exp(const_list$log_params_max[names(const_list$log_params_max)=="p_rep_severe"])){prior_report=-Inf}
  } else {
  ...
  if(is.numeric(const_list$p_rep_death)==FALSE){
    ...
    if(p_rep_death<exp(const_list$log_params_min[names(const_list$log_params_min)=="p_rep_death"])){prior_report=-Inf}
    if(p_rep_death>exp(const_list$log_params_max[names(const_list$log_params_max)=="p_rep_death"])){prior_report=-Inf}
  } else {
  ...
  if(is.numeric(const_list$m_FOI_Brazil)==FALSE){
    ...
    if(m_FOI_Brazil<exp(const_list$log_params_min[names(const_list$log_params_min)=="m_FOI_Brazil"])){prior_report=-Inf}
    if(m_FOI_Brazil>exp(const_list$log_params_max[names(const_list$log_params_max)=="m_FOI_Brazil"])){prior_report=-Inf}
  } else {
    m_FOI_Brazil=const_list$m_FOI_Brazil
  }

  #Get FOI and R0 values and calculate associated prior
  FOI_R0_data=mcmc_FOI_R0_setup(const_list$type,const_list$prior_type,regions,log_params_prop,const_list$enviro_data,
                                const_list$R0_fixed_values,const_list$log_params_min,const_list$log_params_max)
