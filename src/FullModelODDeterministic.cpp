...
[[cpp11::register]]
SEXP dust_cpu_FullModelODDeterministic_alloc(cpp11::list r_pars, bool pars_multi, size_t step,
...
[[cpp11::register]]
SEXP dust_cpu_FullModelODDeterministic_update_state(SEXP ptr, SEXP r_pars, SEXP r_state,
...
[[cpp11::register]]
SEXP dust_cpu_FullModelODDeterministic_filter(SEXP ptr, SEXP step_end,
...
// [[dust::class(FullModelODDeterministic)]]
...
class FullModelODDeterministic {
public:
  ...
  FullModelODDeterministic(const dust::pars_type<FullModelODDeterministic>& pars) :
  ...
  std::vector<real_type> initial(size_t step) {
    std::vector<real_type> state(shared->dim_C + shared->dim_E + shared->dim_I + shared->dim_R + shared->dim_S + shared->dim_V + 4);
  ...
  void update(size_t step, const real_type * state, rng_state_type& rng_state, real_type * state_next) {
    ...
    {
       int i = 1;
       state_next[4 + i - 1] = std::max(shared->Pmin, S[0] - internal.E_new[0] - internal.vacc_rate[0] * S[0] * internal.inv_P_nV[0] + internal.dP1[0] - (internal.dP2[0] * S[0] * internal.inv_P[0]));
    }
    for (int i = 2; i <= shared->N_age; ++i) {
      state_next[4 + i - 1] = std::max(shared->Pmin, S[i - 1] - internal.E_new[i - 1] - internal.vacc_rate[i - 1] * S[i - 1] * internal.inv_P_nV[i - 1] + (internal.dP1[i - 1] * S[i - 1 - 1] * internal.inv_P[i - 1 - 1]) - (internal.dP2[i - 1] * S[i - 1] * internal.inv_P[i - 1]));
    }
  }
private:
...
namespace dust {
template<>
dust::pars_type<FullModelODDeterministic> dust_pars<FullModelODDeterministic>(cpp11::list user) {
  using real_type = typename FullModelODDeterministic::real_type;
  auto shared = std::make_shared<FullModelODDeterministic::shared_type>();
  FullModelODDeterministic::internal_type internal;
  shared->FOI_max = 1;
  ...
  return dust::pars_type<FullModelODDeterministic>(shared, internal);
}
template <>
cpp11::sexp dust_info<FullModelODDeterministic>(const dust::pars_type<FullModelODDeterministic>& pars) {
  const FullModelODDeterministic::internal_type internal = pars.internal;
  const std::shared_ptr<const FullModelODDeterministic::shared_type> shared = pars.shared;
...
cpp11::sexp dust_FullModelODDeterministic_capabilities() {
  return dust::r::dust_capabilities<FullModelODDeterministic>();
}

cpp11::sexp dust_FullModelODDeterministic_gpu_info() {
  return dust::gpu::r::gpu_info();
}
using model_cpu = dust::dust_cpu<FullModelODDeterministic>;

SEXP dust_cpu_FullModelODDeterministic_alloc(cpp11::list r_pars, bool pars_multi, size_t step,
                             cpp11::sexp r_n_particles, size_t n_threads,
                             cpp11::sexp r_seed, bool deterministic,
                             cpp11::sexp gpu_config) {
  return dust::r::dust_cpu_alloc<FullModelODDeterministic>(r_pars, pars_multi, step, r_n_particles,
...
SEXP dust_cpu_FullModelODDeterministic_set_data(SEXP ptr, cpp11::list data,
                                       bool shared) {
