...
Population and vaccination data is stored in a list object as described in [Guide 2 - Inputs](https://mrc-ide.github.io/YellowFeverDynamics/articles/BGuideBInputs.html). Here an example data set for multiple regions is loaded and the vaccination and population data for the first included region is extracted:
...
For some applications, the Basic_Model_Run() function can be used instead. This is largely identical to Full_Model_Run() but does not output the total force of infection at each time point or the number of new infections at each time point. This reduces the amount of data output by each run of the function, reducing computational requirements. Basic_Model_Run is used, for example, when the intention is only to calculate seroprevalence, which is derived (as described in [Guide 4 - Processing Model Results](https://mrc-ide.github.io/YellowFeverDynamics/articles/BGuideDModelResultsProcess.html)) from SEIRV values, with the rate of newly infectious individuals being irrelevant.
