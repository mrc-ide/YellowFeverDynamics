...
We set additional inputs as follows. The MCMC process uses the natural logarithm values of the actual parameters varied in the chain; the inputs pars_min, pars_max and pars_ini are set accordingly. The maximum, minimum and initial values of the varied parameters must be entered into pars_min, pars_max and pars_ini in the following order:
...
pars_min <- c(rep(-30, n_env_vars), rep(-5, n_env_vars), log(c(0.9,0.01, 0.01))) # Minimum log parameter values (here FOI coefficients, R0 coefficients, vaccine efficacy, severe non-fatal case reporting probability, fatal case reporting probability)
pars_max <- c(rep(-10, n_env_vars), rep(0, n_env_vars), log(c(1.0,0.25, 0.25))) # Maximum log parameter values
pars_ini <- c(log(enviro_coeffs_ini), log(c(0.95,0.1,0.2))) # Initial log parameter values
...
```{r,eval=FALSE}
MCMC(pars_ini, input_data, obs_sero_data, obs_case_data, obs_outbreak_data, filename_prefix, Niter, type, pars_min,
     pars_max, n_reps, mode_start, prior_type, dt, enviro_data, R0_fixed_values, vaccine_efficacy, p_rep_severe,
     p_rep_death)
```
