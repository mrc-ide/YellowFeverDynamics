...
We load input data, environmental data and the observed data to be used for comparison in the formats used in previous worked examples (see [Guide 1 - Inputs](https://mrc-ide.github.io/YellowFeverDynamics/articles/CGuideAInputs.html) for details of the input format, [Guide 2 - Calculating Parameters From Environmental Data](https://mrc-ide.github.io/YellowFeverDynamics/articles/CGuideBParametersEnvironment.html) for details of how environmental data is formatted and used to generate parameter values, and [Worked Example 3 - Generating Epidemiological Data Sets](https://mrc-ide.github.io/YellowFeverDynamics/articles/BWorkedExampleCDataGenerate.html) for details of observed data formats):
...
obs_sero_data <- read.csv(file = paste(path.package("YellowFeverDynamics"), "/exdata/obs_sero_data_example.csv", 
                                       sep = ""), header = TRUE)
obs_case_data <- read.csv(file = paste(path.package("YellowFeverDynamics"), "/exdata/obs_case_data_example.csv", 
                                       sep = ""), header = TRUE)
obs_outbreak_data <- NULL
enviro_coeffs_ini <- as.numeric(read.csv(file = paste(path.package("YellowFeverDynamics"),
...
type <- "FOI+R0 enviro" # Type of parameter set used to calculate spillover FOI and R0 (here both calculated from
                        # environmental covariates)
log_params_min <- c(rep(-30, n_env_vars), rep(-5, n_env_vars), log(c(0.01, 0.01))) 
               # Minimum log parameter values (here FOI coefficients, R0 coefficients, vaccine efficacy, severe
               # non-fatal case reporting probability, fatal case reporting probability)
...
R0_fixed_values = p_rep_severe = p_rep_death = NULL # Fixed R0 values and severe and fatal case reporting probabilities;
                                                    # here set to NULL because values are variable
filename_prefix <- "Chain" # Prefix to be used for .csv output files; a new file is created every 10,000 iterations with
                           # incremental number suffixes (Chain0.csv, Chain1.csv, etc.)
```
...
```{r,eval=FALSE}
chain_example <- read.csv(file = paste(path.package("YellowFeverDynamics"), "/exdata/Chain_example.csv", sep = ""),
                          header = TRUE)
