...
// [[dust2::class(SEIRVModelDelay_new)]]
...
class SEIRVModelDelay_new {
public:
  SEIRVModelDelay_new() = delete;
  ...
  static shared_state build_shared(cpp11::list parameters) {
    ...
    return shared_state{odin, dim, time_inc, n_regions, t_infectious, N_age, vaccine_efficacy, year0, n_years, n_t_pts, Pmin, FOI_max, np_E_delay, np_I_delay, di1, di2, FOI_spillover, R0, vacc_rate_daily, S_0, E_0, E_delay0, I_0, I_delay0, R_0, V_0, dP1_all, dP2_all};
  }
  ...
  static void initial(real_type time, const shared_state& shared, internal_state& internal, rng_state_type& rng_state, real_type* state) {
    ...
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.np_E_delay); ++j) {
        state[i - 1 + (j - 1) * shared.dim.E_delay.mult[1] + shared.odin.offset.state[5]] = shared.E_delay0[i - 1 + (j - 1) * shared.dim.E_delay0.mult[1]];
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        state[i - 1 + (j - 1) * shared.dim.I.mult[1] + shared.odin.offset.state[6]] = shared.I_0[i - 1 + (j - 1) * shared.dim.I_0.mult[1]];
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.np_I_delay); ++j) {
        state[i - 1 + (j - 1) * shared.dim.I_delay.mult[1] + shared.odin.offset.state[7]] = shared.I_delay0[i - 1 + (j - 1) * shared.dim.I_delay0.mult[1]];
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        state[i - 1 + (j - 1) * shared.dim.R.mult[1] + shared.odin.offset.state[8]] = shared.R_0[i - 1 + (j - 1) * shared.dim.R_0.mult[1]];
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        state[i - 1 + (j - 1) * shared.dim.V.mult[1] + shared.odin.offset.state[9]] = shared.V_0[i - 1 + (j - 1) * shared.dim.V_0.mult[1]];
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        state[i - 1 + (j - 1) * shared.dim.C.mult[1] + shared.odin.offset.state[10]] = 0;
      }
    }
  }
  static void update(real_type time, real_type dt, const real_type* state, const shared_state& shared, internal_state& internal, rng_state_type& rng_state, real_type* state_next) {
    ...
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = (shared.N_age + 1); j <= static_cast<size_t>(shared.np_E_delay); ++j) {
        state_next[i - 1 + (j - 1) * shared.dim.E_delay.mult[1] + shared.odin.offset.state[5]] = E_delay[i - 1 + (j - shared.N_age - 1) * shared.dim.E_delay.mult[1]];
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        state_next[i - 1 + (j - 1) * shared.dim.E_delay.mult[1] + shared.odin.offset.state[5]] = internal.E_new[i - 1 + (j - 1) * shared.dim.E_new.mult[1]];
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        state_next[i - 1 + (j - 1) * shared.dim.I.mult[1] + shared.odin.offset.state[6]] = monty::math::max<real_type>(shared.Pmin, I[i - 1 + (j - 1) * shared.dim.I.mult[1]] + internal.I_new[i - 1 + (j - 1) * shared.dim.I_new.mult[1]] - internal.R_new[i - 1 + (j - 1) * shared.dim.R_new.mult[1]]);
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = (shared.N_age + 1); j <= static_cast<size_t>(shared.np_I_delay); ++j) {
        state_next[i - 1 + (j - 1) * shared.dim.I_delay.mult[1] + shared.odin.offset.state[7]] = I_delay[i - 1 + (j - shared.N_age - 1) * shared.dim.I_delay.mult[1]];
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        state_next[i - 1 + (j - 1) * shared.dim.I_delay.mult[1] + shared.odin.offset.state[7]] = internal.I_new[i - 1 + (j - 1) * shared.dim.I_new.mult[1]];
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      state_next[i - 1 + shared.odin.offset.state[8]] = monty::math::max<real_type>(shared.Pmin, R[i - 1] + internal.R_new[i - 1] - internal.vacc_rate[i - 1] * R[i - 1] * internal.inv_P_nV[i - 1] - (internal.dP2[i - 1] * R[i - 1] * internal.inv_P[i - 1]));
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 2; j <= static_cast<size_t>(shared.N_age); ++j) {
        state_next[i - 1 + (j - 1) * shared.dim.R.mult[1] + shared.odin.offset.state[8]] = monty::math::max<real_type>(shared.Pmin, R[i - 1 + (j - 1) * shared.dim.R.mult[1]] + internal.R_new[i - 1 + (j - 1) * shared.dim.R_new.mult[1]] - internal.vacc_rate[i - 1 + (j - 1) * shared.dim.vacc_rate.mult[1]] * R[i - 1 + (j - 1) * shared.dim.R.mult[1]] * internal.inv_P_nV[i - 1 + (j - 1) * shared.dim.inv_P_nV.mult[1]] + (internal.dP1[i - 1 + (j - 1) * shared.dim.dP1.mult[1]] * R[i - 1 + (j - 1 - 1) * shared.dim.R.mult[1]] * internal.inv_P[i - 1 + (j - 1 - 1) * shared.dim.inv_P.mult[1]]) - (internal.dP2[i - 1 + (j - 1) * shared.dim.dP2.mult[1]] * R[i - 1 + (j - 1) * shared.dim.R.mult[1]] * internal.inv_P[i - 1 + (j - 1) * shared.dim.inv_P.mult[1]]));
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      state_next[i - 1 + shared.odin.offset.state[9]] = monty::math::max<real_type>(shared.Pmin, V[i - 1] + internal.vacc_rate[i - 1] - (internal.dP2[i - 1] * V[i - 1] * internal.inv_P[i - 1]));
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 2; j <= static_cast<size_t>(shared.N_age); ++j) {
        state_next[i - 1 + (j - 1) * shared.dim.V.mult[1] + shared.odin.offset.state[9]] = monty::math::max<real_type>(shared.Pmin, V[i - 1 + (j - 1) * shared.dim.V.mult[1]] + internal.vacc_rate[i - 1 + (j - 1) * shared.dim.vacc_rate.mult[1]] + (internal.dP1[i - 1 + (j - 1) * shared.dim.dP1.mult[1]] * V[i - 1 + (j - 1 - 1) * shared.dim.V.mult[1]] * internal.inv_P[i - 1 + (j - 1 - 1) * shared.dim.inv_P.mult[1]]) - (internal.dP2[i - 1 + (j - 1) * shared.dim.dP2.mult[1]] * V[i - 1 + (j - 1) * shared.dim.V.mult[1]] * internal.inv_P[i - 1 + (j - 1) * shared.dim.inv_P.mult[1]]));
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        state_next[i - 1 + (j - 1) * shared.dim.C.mult[1] + shared.odin.offset.state[10]] = internal.I_new[i - 1 + (j - 1) * shared.dim.I_new.mult[1]];
      }
    }
  }
};
...
SEXP dust2_system_SEIRVModelDelay_new_alloc(cpp11::list r_pars, cpp11::sexp r_time, cpp11::list r_time_control, cpp11::sexp r_n_particles, cpp11::sexp r_n_groups, cpp11::sexp r_seed, cpp11::sexp r_deterministic, cpp11::sexp r_n_threads) {
  return dust2::r::dust2_discrete_alloc<SEIRVModelDelay_new>(r_pars, r_time, r_time_control, r_n_particles, r_n_groups, r_seed, r_deterministic, r_n_threads);
}
[[cpp11::register]]
SEXP dust2_system_SEIRVModelDelay_new_run_to_time(cpp11::sexp ptr, cpp11::sexp r_time) {
  return dust2::r::dust2_system_run_to_time<dust2::dust_discrete<SEIRVModelDelay_new>>(ptr, r_time);
}
...
SEXP dust2_system_SEIRVModelDelay_new_state(cpp11::sexp ptr, cpp11::sexp r_index_state, cpp11::sexp r_index_particle, cpp11::sexp r_index_group, bool preserve_particle_dimension, bool preserve_group_dimension) {
  return dust2::r::dust2_system_state<dust2::dust_discrete<SEIRVModelDelay_new>>(ptr, r_index_state, r_index_particle, r_index_group, preserve_particle_dimension, preserve_group_dimension);
}

[[cpp11::register]]
SEXP dust2_system_SEIRVModelDelay_new_time(cpp11::sexp ptr) {
  return dust2::r::dust2_system_time<dust2::dust_discrete<SEIRVModelDelay_new>>(ptr);
}

[[cpp11::register]]
SEXP dust2_system_SEIRVModelDelay_new_set_state_initial(cpp11::sexp ptr) {
  return dust2::r::dust2_system_set_state_initial<dust2::dust_discrete<SEIRVModelDelay_new>>(ptr);
}

[[cpp11::register]]
SEXP dust2_system_SEIRVModelDelay_new_set_state(cpp11::sexp ptr, cpp11::list r_state) {
  return dust2::r::dust2_system_set_state<dust2::dust_discrete<SEIRVModelDelay_new>>(ptr, r_state);
}

[[cpp11::register]]
SEXP dust2_system_SEIRVModelDelay_new_reorder(cpp11::sexp ptr, cpp11::integers r_index) {
  return dust2::r::dust2_system_reorder<dust2::dust_discrete<SEIRVModelDelay_new>>(ptr, r_index);
}

[[cpp11::register]]
SEXP dust2_system_SEIRVModelDelay_new_rng_state(cpp11::sexp ptr) {
  return dust2::r::dust2_system_rng_state<dust2::dust_discrete<SEIRVModelDelay_new>>(ptr);
}

[[cpp11::register]]
SEXP dust2_system_SEIRVModelDelay_new_set_rng_state(cpp11::sexp ptr, cpp11::sexp r_rng_state) {
  return dust2::r::dust2_system_set_rng_state<dust2::dust_discrete<SEIRVModelDelay_new>>(ptr, r_rng_state);
}

[[cpp11::register]]
SEXP dust2_system_SEIRVModelDelay_new_set_time(cpp11::sexp ptr, cpp11::sexp r_time) {
  return dust2::r::dust2_system_set_time<dust2::dust_discrete<SEIRVModelDelay_new>>(ptr, r_time);
}

[[cpp11::register]]
SEXP dust2_system_SEIRVModelDelay_new_update_pars(cpp11::sexp ptr, cpp11::list pars) {
  return dust2::r::dust2_system_update_pars<dust2::dust_discrete<SEIRVModelDelay_new>>(ptr, pars);
}
...
SEXP dust2_system_SEIRVModelDelay_new_simulate(cpp11::sexp ptr, cpp11::sexp r_times, cpp11::sexp r_index_state, bool preserve_particle_dimension, bool preserve_group_dimension) {
  return dust2::r::dust2_system_simulate<dust2::dust_discrete<SEIRVModelDelay_new>>(ptr, r_times, r_index_state, preserve_particle_dimension, preserve_group_dimension);
}
