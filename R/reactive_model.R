...
#' @param p_rep Probability of an infection being reported as a case
#' '
#' @export
#'
Reactive_Model_Run <- function(FOI_spillover=0.0,R0=1.0,vacc_data=list(),pop_data=list(),year0=1940,mode_start=0,
                           n_particles=1,n_threads=1,year_end=2000,year_data_begin=1999,vaccine_efficacy=1.0,
                           start_SEIRV=list(),dt=1.0,p_rep=1.0e-6) {
  ...
  x <- ReactiveModelOD$new(pars=parameter_setup2(FOI_spillover,R0,vacc_data,pop_data,year0,mode_start,year_end,
                                            year_data_begin,vaccine_efficacy,start_SEIRV,dt,p_rep),
                       step = 1,n_particles = n_particles,n_threads = n_threads)
  
  n_nv=5 #Number of non-vector outputs at beginning of output
...
#' @export
#'
parameter_setup2 <- function(FOI_spillover=0.0,R0=1.0,vacc_data=list(),pop_data=list(),year0=1940,mode_start=0,
