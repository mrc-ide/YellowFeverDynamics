...
param_dist <- read.csv(file = paste(path.package("YellowFeverDynamics"), "/exdata/enviro_coeffs_example_multi.csv", sep = ""), header = TRUE) # Data frame containing one or more sets of parameter values (here the actual values are used, rather than the logarithmic values as in the case of the data-matching functions)
...
mode_start <- 1 # Flag indicating how to set initial population immunity level in addition to vaccination
                ...
                # If mode_start = 2, use SEIRV input in list from previous run(s), set using start_SEIRV0 variable
...
results<-total_burden_estimate(type,param_dist,input_data,start_SEIRV0=NULL,years_data,n_reps,mode_start,flag_reporting,
                               dt,enviro_data,R0_fixed_values,vaccine_efficacy0,p_rep_severe0,p_rep_death0)
```
...
results_novacc<-total_burden_estimate(type,param_dist,input_data_novacc,start_SEIRV0=NULL,years_data,n_reps,mode_start,
                              flag_reporting,dt,enviro_data,R0_fixed_values,vaccine_efficacy0,p_rep_severe0,p_rep_death0)
deaths_prevented=results_novacc$all$deaths-results$all$deaths # 
```
