...
public:
  ...
  static shared_state build_shared(cpp11::list parameters) {
    ...
    return shared_state{odin, dim, time_inc, t_incubation, t_latent, t_infectious, FOI_spillover, R0, N_age, vaccine_efficacy, response_delay, case_threshold, cluster_threshold, t_cam, year0, n_years, Pmin, FOI_max, one, p_rep, rate1, rate2, rate3, rate4, beta, vacc_rate_daily, vacc_cov_cam, S_0, E_0, I_0, R_0, V_0, dP1_all, dP2_all};
  }
  ...
  static void initial(real_type time, const shared_state& shared, internal_state& internal, rng_state_type& rng_state, real_type* state) {
    ...
    for (size_t i = 1; i <= static_cast<size_t>(shared.N_age); ++i) {
      state[i - 1 + shared.odin.offset.state[11]] = shared.I_0[i - 1];
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.N_age); ++i) {
      state[i - 1 + shared.odin.offset.state[12]] = shared.R_0[i - 1];
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.N_age); ++i) {
      state[i - 1 + shared.odin.offset.state[13]] = shared.V_0[i - 1];
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.N_age); ++i) {
  ...
  static void update(real_type time, real_type dt, const real_type* state, const shared_state& shared, internal_state& internal, rng_state_type& rng_state, real_type* state_next) {
    ...
    const auto * I = state + shared.odin.offset.state[11];
    const auto * R = state + shared.odin.offset.state[12];
    const auto * V = state + shared.odin.offset.state[13];
    ...
    for (size_t i = 1; i <= static_cast<size_t>(shared.N_age); ++i) {
      state_next[i - 1 + shared.odin.offset.state[11]] = monty::math::max(shared.Pmin, I[i - 1] + internal.I_new[i - 1] - internal.R_new[i - 1]);
    }
    state_next[shared.odin.offset.state[12]] = monty::math::max(shared.Pmin, R[0] + internal.R_new[0] - internal.vacc_rate[0] * R[0] * internal.inv_P_nV[0] - (internal.dP2[0] * R[0] * internal.inv_P[0]));
    for (size_t i = 2; i <= static_cast<size_t>(shared.N_age); ++i) {
      state_next[i - 1 + shared.odin.offset.state[12]] = monty::math::max(shared.Pmin, R[i - 1] + internal.R_new[i - 1] - internal.vacc_rate[i - 1] * R[i - 1] * internal.inv_P_nV[i - 1] + (internal.dP1[i - 1] * R[i - 1 - 1] * internal.inv_P[i - 1 - 1]) - (internal.dP2[i - 1] * R[i - 1] * internal.inv_P[i - 1]));
    }
    state_next[shared.odin.offset.state[13]] = monty::math::max(shared.Pmin, V[0] + internal.vacc_rate[0] - (internal.dP2[0] * V[0] * internal.inv_P[0]));
    for (size_t i = 2; i <= static_cast<size_t>(shared.N_age); ++i) {
      state_next[i - 1 + shared.odin.offset.state[13]] = monty::math::max(shared.Pmin, V[i - 1] + internal.vacc_rate[i - 1] + (internal.dP1[i - 1] * V[i - 1 - 1] * internal.inv_P[i - 1 - 1]) - (internal.dP2[i - 1] * V[i - 1] * internal.inv_P[i - 1]));
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.N_age); ++i) {
      state_next[i - 1 + shared.odin.offset.state[14]] = internal.I_new[i - 1];
    }
  }
};
...
SEXP dust2_system_SEIRVModelReactive_alloc(cpp11::list r_pars, cpp11::sexp r_time, cpp11::list r_time_control, cpp11::sexp r_n_particles, cpp11::sexp r_n_groups, cpp11::sexp r_seed, cpp11::sexp r_deterministic, cpp11::sexp r_n_threads) {
  return dust2::r::dust2_discrete_alloc<SEIRVModelReactive>(r_pars, r_time, r_time_control, r_n_particles, r_n_groups, r_seed, r_deterministic, r_n_threads);
}
[[cpp11::register]]
SEXP dust2_system_SEIRVModelReactive_run_to_time(cpp11::sexp ptr, cpp11::sexp r_time) {
  return dust2::r::dust2_system_run_to_time<dust2::dust_discrete<SEIRVModelReactive>>(ptr, r_time);
}
...
SEXP dust2_system_SEIRVModelReactive_state(cpp11::sexp ptr, cpp11::sexp r_index_state, cpp11::sexp r_index_particle, cpp11::sexp r_index_group, bool preserve_particle_dimension, bool preserve_group_dimension) {
  return dust2::r::dust2_system_state<dust2::dust_discrete<SEIRVModelReactive>>(ptr, r_index_state, r_index_particle, r_index_group, preserve_particle_dimension, preserve_group_dimension);
}

[[cpp11::register]]
SEXP dust2_system_SEIRVModelReactive_time(cpp11::sexp ptr) {
  return dust2::r::dust2_system_time<dust2::dust_discrete<SEIRVModelReactive>>(ptr);
}

[[cpp11::register]]
SEXP dust2_system_SEIRVModelReactive_set_state_initial(cpp11::sexp ptr) {
  return dust2::r::dust2_system_set_state_initial<dust2::dust_discrete<SEIRVModelReactive>>(ptr);
}

[[cpp11::register]]
SEXP dust2_system_SEIRVModelReactive_set_state(cpp11::sexp ptr, cpp11::list r_state) {
  return dust2::r::dust2_system_set_state<dust2::dust_discrete<SEIRVModelReactive>>(ptr, r_state);
}

[[cpp11::register]]
SEXP dust2_system_SEIRVModelReactive_reorder(cpp11::sexp ptr, cpp11::integers r_index) {
  return dust2::r::dust2_system_reorder<dust2::dust_discrete<SEIRVModelReactive>>(ptr, r_index);
}

[[cpp11::register]]
SEXP dust2_system_SEIRVModelReactive_rng_state(cpp11::sexp ptr) {
  return dust2::r::dust2_system_rng_state<dust2::dust_discrete<SEIRVModelReactive>>(ptr);
}

[[cpp11::register]]
SEXP dust2_system_SEIRVModelReactive_set_rng_state(cpp11::sexp ptr, cpp11::sexp r_rng_state) {
  return dust2::r::dust2_system_set_rng_state<dust2::dust_discrete<SEIRVModelReactive>>(ptr, r_rng_state);
}

[[cpp11::register]]
SEXP dust2_system_SEIRVModelReactive_set_time(cpp11::sexp ptr, cpp11::sexp r_time) {
  return dust2::r::dust2_system_set_time<dust2::dust_discrete<SEIRVModelReactive>>(ptr, r_time);
}

[[cpp11::register]]
SEXP dust2_system_SEIRVModelReactive_update_pars(cpp11::sexp ptr, cpp11::list pars) {
  return dust2::r::dust2_system_update_pars<dust2::dust_discrete<SEIRVModelReactive>>(ptr, pars);
}
...
SEXP dust2_system_SEIRVModelReactive_simulate(cpp11::sexp ptr, cpp11::sexp r_times, cpp11::sexp r_index_state, bool preserve_particle_dimension, bool preserve_group_dimension) {
  return dust2::r::dust2_system_simulate<dust2::dust_discrete<SEIRVModelReactive>>(ptr, r_times, r_index_state, preserve_particle_dimension, preserve_group_dimension);
}
