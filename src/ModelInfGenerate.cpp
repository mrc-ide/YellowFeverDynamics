...
[[cpp11::register]]
SEXP dust_cpu_ModelInfGenerate_alloc(cpp11::list r_pars, bool pars_multi, size_t step,
...
[[cpp11::register]]
SEXP dust_cpu_ModelInfGenerate_update_state(SEXP ptr, SEXP r_pars, SEXP r_state,
...
[[cpp11::register]]
SEXP dust_cpu_ModelInfGenerate_filter(SEXP ptr, SEXP step_end,
...
public:
  ...
  struct internal_type {
    ...
    std::vector<real_type> I_new1;
    std::vector<real_type> I_new2;
  ...
  ModelInfGenerate(const dust::pars_type<ModelInfGenerate>& pars) :
  ...
  void update(size_t step, const real_type * state, rng_state_type& rng_state, real_type * state_next) {
    ...
    for (int i = 1; i <= shared->N_age; ++i) {
      internal.I_new1[i - 1] = E[i - 1] * shared->rate1;
    }
    for (int i = 1; i <= shared->N_age; ++i) {
    ...
    for (int i = 1; i <= shared->N_age; ++i) {
      internal.I_new2[i - 1] = std::floor(internal.I_new1[i - 1]) + dust::random::binomial<real_type>(rng_state, static_cast<int>(1), fmodr<real_type>(internal.I_new1[i - 1], 1));
    }
    for (int i = 1; i <= shared->N_age; ++i) {
    ...
    for (int i = 1; i <= shared->N_age; ++i) {
      state_next[shared->offset_variable_C + i - 1] = internal.I_new2[i - 1];
    }
    for (int i = 1; i <= shared->N_age; ++i) {
      state_next[shared->offset_variable_I + i - 1] = I[i - 1] + internal.I_new2[i - 1] - internal.R_new[i - 1];
    }
    for (int i = 1; i <= shared->N_age; ++i) {
    ...
    for (int i = 1; i <= shared->N_age; ++i) {
      state_next[shared->offset_variable_E + i - 1] = E[i - 1] + internal.E_new[i - 1] - internal.I_new2[i - 1];
    }
    {
       int i = 1;
...
namespace dust {
template<>
dust::pars_type<ModelInfGenerate> dust_pars<ModelInfGenerate>(cpp11::list user) {
  using real_type = typename ModelInfGenerate::real_type;
  auto shared = std::make_shared<ModelInfGenerate::shared_type>();
  ModelInfGenerate::internal_type internal;
  shared->FOI_max = 1;
  ...
  shared->dim_I_new1 = shared->N_age;
  shared->dim_I_new2 = shared->N_age;
  ...
  internal.I_new1 = std::vector<real_type>(shared->dim_I_new1);
  internal.I_new2 = std::vector<real_type>(shared->dim_I_new2);
  ...
  return dust::pars_type<ModelInfGenerate>(shared, internal);
}
template <>
cpp11::sexp dust_info<ModelInfGenerate>(const dust::pars_type<ModelInfGenerate>& pars) {
  const ModelInfGenerate::internal_type internal = pars.internal;
  const std::shared_ptr<const ModelInfGenerate::shared_type> shared = pars.shared;
  cpp11::writable::strings nms({"time", "day", "year", "S", "E", "I", "R", "V", "C"});
...
cpp11::sexp dust_ModelInfGenerate_capabilities() {
  return dust::r::dust_capabilities<ModelInfGenerate>();
}

cpp11::sexp dust_ModelInfGenerate_gpu_info() {
  return dust::gpu::r::gpu_info();
}
using model_cpu = dust::dust_cpu<ModelInfGenerate>;

SEXP dust_cpu_ModelInfGenerate_alloc(cpp11::list r_pars, bool pars_multi, size_t step,
                             cpp11::sexp r_n_particles, size_t n_threads,
                             cpp11::sexp r_seed, bool deterministic,
                             cpp11::sexp gpu_config) {
  return dust::r::dust_cpu_alloc<ModelInfGenerate>(r_pars, pars_multi, step, r_n_particles,
...
SEXP dust_cpu_ModelInfGenerate_set_data(SEXP ptr, cpp11::list data,
                                       bool shared) {
