...
Full_Model_Run_Deterministic <- function(FOI_spillover=0.0,R0=1.0,vacc_data=list(),pop_data=list(),year0=1940,
                                         mode_start=0,year_end=2000,year_data_begin=1999,
  ...
  x <- FullModelOD$new(pars=parameter_setup(FOI_spillover,R0,vacc_data,pop_data,year0,mode_start,year_end,
                       ...
                       time = 1,n_particles = 1,n_threads = 1,deterministic=TRUE)
  ...
  x_res <- array(NA, dim = c(n_data_pts, 1, t_pts_out))
  for(t in step0:n_steps){
    x_res[,,t-step0] <- x$run(t)
  }

  return(list(day=array(x_res[2,,],dim=c(1,t_pts_out)),
              year=array(x_res[3,,],dim=c(1,t_pts_out)),
              FOI_total=array(x_res[4,,],dim=c(1,t_pts_out)),
              S=array(x_res[c((1+n_nv):(N_age+n_nv)),,],dim=c(N_age,1,t_pts_out)),
              E=array(x_res[c((N_age+1+n_nv):((2*N_age)+n_nv)),,],dim=c(N_age,1,t_pts_out)),
              I=array(x_res[c(((2*N_age)+1+n_nv):((3*N_age)+n_nv)),,],dim=c(N_age,1,t_pts_out)),
              R=array(x_res[c(((3*N_age)+1+n_nv):((4*N_age)+n_nv)),,],dim=c(N_age,1,t_pts_out)),
              V=array(x_res[c(((4*N_age)+1+n_nv):((5*N_age)+n_nv)),,],dim=c(N_age,1,t_pts_out)),
              C=array(x_res[c(((5*N_age)+1+n_nv):((6*N_age)+n_nv)),,],dim=c(N_age,1,t_pts_out))))
}
#-------------------------------------------------------------------------------
...
#'
#' @param FOI_spillover = Force of infection due to spillover from sylvatic reservoir
#' @param R0 Basic eproduction number for urban spread of infection
#' @param vacc_data Vaccination coverage in each age group by year
#' @param pop_data Population in each age group by year
#' @param year0 First year in population/vaccination data
#' @param mode_start Flag indicating how to set initial population immunity level in addition to vaccination
...
case_data_generate_deterministic <- function(FOI_spillover=0.0,R0=1.0,vacc_data=list(),pop_data=list(),year0=1940,
  ...
  steps=n_steps-step0
  results_data=list(year=sort(rep(c(year_data_begin:(year_end-1)),(365/dt))),C=array(data=rep(0,1*steps),
                                                                                     dim=c(1,steps)))
  pts_select=c(((5*N_age)+1+n):((6*N_age)+n))

  x <- FullModelOD$new(pars=pars,time = 1,n_particles = 1,n_threads = 1,deterministic=TRUE)

  x_res <- array(NA, dim = c(n_data_pts, 1))
  for(t in step0:n_steps){
    x_res <- x$run(t)
    results_data$C[1,t-step0]=colSums(x_res[pts_select,1],dims=1)
  }
