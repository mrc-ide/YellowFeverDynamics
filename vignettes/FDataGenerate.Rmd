...
The inputs for the data_match_single() function, which is used to generate model data to match observed or dummy data, are as follows:
...
params <- c(FOI_values,R0_values) #TBA
...
const_list <- list(type="FOI+R0",n_reps=1,mode_start=1,dt=1.0,enviro_data=NULL,R0_fixed_values=NULL,vaccine_efficacy=1.0,p_rep_severe=1.0,p_rep_death=1.0)
                      # List containing additional parameters (TBA)
```

The data_match_single() function is run to produce a set of modelled data matching the observed data:

```{r,eval=FALSE}
data <- data_match_single(params,input_data,obs_sero_data,obs_case_data,obs_outbreak_data,const_list)
#[TBA]
```

A data_match_multi() function exists to run data_match_single() for a series of sets of values of the input parameters. The inputs are the same except that the param variable can be a data frame containing one set of parameter values per line.

```{r,eval=FALSE}
data <- data_match_multi(params,input_data,obs_sero_data,obs_case_data,obs_outbreak_data,const_list)
#[TBA]
