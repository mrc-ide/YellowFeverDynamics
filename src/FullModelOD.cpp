...
#include <cpp11.hpp>
[[cpp11::register]]
SEXP dust_FullModelOD_alloc(cpp11::list r_pars, bool pars_multi, size_t step,
...
[[cpp11::register]]
SEXP dust_FullModelOD_filter(SEXP ptr, bool save_trajectories,
...
public:
  ...
  struct shared_t {
    ...
    std::vector<real_t> dP1_all;
    std::vector<real_t> dP2_all;
    real_t dt;
    std::vector<real_t> Exp0;
    real_t FOI_max;
    real_t FOI_spillover;
    std::vector<real_t> Inf0;
    std::vector<real_t> initial_C;
    real_t initial_day;
    std::vector<real_t> initial_E;
    real_t initial_FOI_total;
    std::vector<real_t> initial_I;
    std::vector<real_t> initial_R;
    std::vector<real_t> initial_S;
    real_t initial_time;
    std::vector<real_t> initial_V;
    real_t initial_year;
    int N_age;
    int n_years;
    ...
    real_t rate1;
    real_t rate2;
    std::vector<real_t> Rec0;
    std::vector<real_t> Sus0;
    real_t t_incubation;
    real_t t_infectious;
    real_t t_latent;
    std::vector<real_t> Vac0;
    std::vector<real_t> vacc_rate_annual;
    real_t vaccine_efficacy;
    real_t year0;
  };
  struct internal_t {
    std::vector<real_t> dP1;
    std::vector<real_t> dP2;
    std::vector<real_t> E_new;
    std::vector<real_t> F_R;
    std::vector<real_t> F_S;
    std::vector<real_t> F_V;
    std::vector<real_t> I_new;
    std::vector<real_t> inv_P;
    std::vector<real_t> P;
    std::vector<real_t> R_new;
    std::vector<real_t> vacc_rate;
  };
  FullModelOD(const dust::pars_t<FullModelOD>& pars) :
    shared(pars.shared), internal(pars.internal) {
  }
  size_t size() {
    return shared->dim_C + shared->dim_E + shared->dim_I + shared->dim_R + shared->dim_S + shared->dim_V + 4;
  }
  std::vector<real_t> initial(size_t step) {
    std::vector<real_t> state(shared->dim_C + shared->dim_E + shared->dim_I + shared->dim_R + shared->dim_S + shared->dim_V + 4);
    state[0] = shared->initial_time;
    state[1] = shared->initial_day;
    state[2] = shared->initial_year;
    state[3] = shared->initial_FOI_total;
  ...
  HOST void update(size_t step, const real_t * state, dust::rng_state_t<real_t>& rng_state, real_t * state_next) {
    const real_t day = state[1];
    const real_t * S = state + 4;
    ...
    const real_t * V = state + shared->offset_variable_V;
    state_next[1] = day + shared->dt;
    state_next[0] = (step + 1) * shared->dt;
    real_t year_i = std::floor((step * shared->dt) / (real_t) 365) + 1;
    ...
    for (int i = 1; i <= shared->N_age; ++i) {
      internal.inv_P[i - 1] = 1 / (real_t) internal.P[i - 1];
    }
    real_t P_tot = odin_sum1<real_t>(internal.P.data(), 0, shared->dim_P);
    for (int i = 1; i <= shared->N_age; ++i) {
    ...
    real_t FOI_sum = std::min(shared->FOI_max, shared->beta * (odin_sum1<real_t>(I, 0, shared->dim_I) / (real_t) P_tot) + (shared->FOI_spillover * shared->dt));
    for (int i = 1; i <= shared->N_age; ++i) {
      internal.E_new[i - 1] = dust::distr::rbinom(rng_state, std::round(static_cast<int>(S[i - 1])), FOI_sum);
    }
    ...
    {
       int i = 1;
       state_next[4 + i - 1] = std::max(shared->Pmin, S[0] - internal.E_new[0] - internal.vacc_rate[0] + internal.dP1[0] - (internal.dP2[0] * internal.F_S[0]));
    }
    for (int i = 2; i <= shared->N_age; ++i) {
      state_next[4 + i - 1] = std::max(shared->Pmin, S[i - 1] - internal.E_new[i - 1] - internal.vacc_rate[i - 1] + (internal.dP1[i - 1] * internal.F_S[i - 1 - 1]) - (internal.dP2[i - 1] * internal.F_S[i - 1]));
    }
  }
private:
...
namespace dust {
template<>
dust::pars_t<FullModelOD> dust_pars<FullModelOD>(cpp11::list user) {
  typedef typename FullModelOD::real_t real_t;
  auto shared = std::make_shared<FullModelOD::shared_t>();
  FullModelOD::internal_t internal;
  shared->FOI_max = 1;
  ...
  shared->FOI_spillover = user_get_scalar<real_t>(user, "FOI_spillover", shared->FOI_spillover, NA_REAL, NA_REAL);
  shared->N_age = user_get_scalar<int>(user, "N_age", shared->N_age, NA_REAL, NA_REAL);
  shared->n_years = user_get_scalar<int>(user, "n_years", shared->n_years, NA_REAL, NA_REAL);
  shared->R0 = user_get_scalar<real_t>(user, "R0", shared->R0, NA_REAL, NA_REAL);
  shared->vaccine_efficacy = user_get_scalar<real_t>(user, "vaccine_efficacy", shared->vaccine_efficacy, NA_REAL, NA_REAL);
  shared->year0 = user_get_scalar<real_t>(user, "year0", shared->year0, NA_REAL, NA_REAL);
  shared->beta = (shared->R0 * shared->dt) / (real_t) shared->t_infectious;
  ...
  shared->rate1 = shared->dt / (real_t) (shared->t_incubation + shared->t_latent);
  shared->rate2 = shared->dt / (real_t) shared->t_infectious;
  internal.dP1 = std::vector<real_t>(shared->dim_dP1);
  internal.dP2 = std::vector<real_t>(shared->dim_dP2);
  internal.E_new = std::vector<real_t>(shared->dim_E_new);
  internal.F_R = std::vector<real_t>(shared->dim_F_R);
  internal.F_S = std::vector<real_t>(shared->dim_F_S);
  internal.F_V = std::vector<real_t>(shared->dim_F_V);
  internal.I_new = std::vector<real_t>(shared->dim_I_new);
  shared->initial_C = std::vector<real_t>(shared->dim_C);
  ...
  shared->initial_V = std::vector<real_t>(shared->dim_V);
  internal.inv_P = std::vector<real_t>(shared->dim_inv_P);
  internal.P = std::vector<real_t>(shared->dim_P);
  internal.R_new = std::vector<real_t>(shared->dim_R_new);
  internal.vacc_rate = std::vector<real_t>(shared->dim_vacc_rate);
  shared->Cas0 = user_get_array_fixed<real_t, 1>(user, "Cas0", shared->Cas0, {shared->dim_Cas0}, NA_REAL, NA_REAL);
  ...
  shared->Exp0 = user_get_array_fixed<real_t, 1>(user, "Exp0", shared->Exp0, {shared->dim_Exp0}, NA_REAL, NA_REAL);
  shared->Inf0 = user_get_array_fixed<real_t, 1>(user, "Inf0", shared->Inf0, {shared->dim_Inf0}, NA_REAL, NA_REAL);
  shared->offset_variable_C = shared->dim_E + shared->dim_I + shared->dim_R + shared->dim_S + shared->dim_V + 4;
  shared->offset_variable_E = shared->dim_S + 4;
  shared->offset_variable_I = shared->dim_E + shared->dim_S + 4;
  shared->offset_variable_R = shared->dim_E + shared->dim_I + shared->dim_S + 4;
  shared->offset_variable_V = shared->dim_E + shared->dim_I + shared->dim_R + shared->dim_S + 4;
  shared->Rec0 = user_get_array_fixed<real_t, 1>(user, "Rec0", shared->Rec0, {shared->dim_Rec0}, NA_REAL, NA_REAL);
  shared->Sus0 = user_get_array_fixed<real_t, 1>(user, "Sus0", shared->Sus0, {shared->dim_Sus0}, NA_REAL, NA_REAL);
  shared->Vac0 = user_get_array_fixed<real_t, 1>(user, "Vac0", shared->Vac0, {shared->dim_Vac0}, NA_REAL, NA_REAL);
  shared->dP1_all = user_get_array_fixed<real_t, 2>(user, "dP1_all", shared->dP1_all, {shared->dim_dP1_all_1, shared->dim_dP1_all_2}, NA_REAL, NA_REAL);
  shared->dP2_all = user_get_array_fixed<real_t, 2>(user, "dP2_all", shared->dP2_all, {shared->dim_dP2_all_1, shared->dim_dP2_all_2}, NA_REAL, NA_REAL);
  for (int i = 1; i <= shared->N_age; ++i) {
  ...
  shared->vacc_rate_annual = user_get_array_fixed<real_t, 2>(user, "vacc_rate_annual", shared->vacc_rate_annual, {shared->dim_vacc_rate_annual_1, shared->dim_vacc_rate_annual_2}, NA_REAL, NA_REAL);
  return dust::pars_t<FullModelOD>(shared, internal);
}
template <>
cpp11::sexp dust_info<FullModelOD>(const dust::pars_t<FullModelOD>& pars) {
  const FullModelOD::internal_t internal = pars.internal;
  const std::shared_ptr<const FullModelOD::shared_t> shared = pars.shared;
  cpp11::writable::strings nms({"time", "day", "year", "FOI_total", "S", "E", "I", "R", "V", "C"});
  ...
  dim[8] = cpp11::writable::integers({shared->dim_V});
  dim[9] = cpp11::writable::integers({shared->dim_C});
  dim.names() = nms;
  ...
  index[8] = integer_sequence(shared->offset_variable_V + 1, shared->dim_V);
  index[9] = integer_sequence(shared->offset_variable_C + 1, shared->dim_C);
  index.names() = nms;
...
SEXP dust_FullModelOD_alloc(cpp11::list r_pars, bool pars_multi, size_t step,
                         cpp11::sexp r_n_particles, size_t n_threads,
                         cpp11::sexp r_seed, cpp11::sexp device_id) {
  return dust::r::dust_alloc<FullModelOD>(r_pars, pars_multi, step, r_n_particles,
                                        n_threads, r_seed, device_id);
}

SEXP dust_FullModelOD_run(SEXP ptr, size_t step_end, bool device) {
  return dust::r::dust_run<FullModelOD>(ptr, step_end, device);
}

SEXP dust_FullModelOD_simulate(SEXP ptr, cpp11::sexp step_end, bool device) {
  return dust::r::dust_simulate<FullModelOD>(ptr, step_end, device);
}

SEXP dust_FullModelOD_set_index(SEXP ptr, cpp11::sexp r_index) {
  dust::r::dust_set_index<FullModelOD>(ptr, r_index);
  return R_NilValue;
}

SEXP dust_FullModelOD_set_state(SEXP ptr, SEXP r_state, SEXP r_step) {
  dust::r::dust_set_state<FullModelOD>(ptr, r_state, r_step);
  return R_NilValue;
}

SEXP dust_FullModelOD_reset(SEXP ptr, cpp11::list r_pars, size_t step) {
  return dust::r::dust_reset<FullModelOD>(ptr, r_pars, step);
}

SEXP dust_FullModelOD_state(SEXP ptr, SEXP r_index) {
  return dust::r::dust_state<FullModelOD>(ptr, r_index);
}

size_t dust_FullModelOD_step(SEXP ptr) {
  return dust::r::dust_step<FullModelOD>(ptr);
}

void dust_FullModelOD_reorder(SEXP ptr, cpp11::sexp r_index) {
  return dust::r::dust_reorder<FullModelOD>(ptr, r_index);
}

SEXP dust_FullModelOD_resample(SEXP ptr, cpp11::doubles r_weights) {
  return dust::r::dust_resample<FullModelOD>(ptr, r_weights);
}

SEXP dust_FullModelOD_set_pars(SEXP ptr, cpp11::list r_pars) {
  return dust::r::dust_set_pars<FullModelOD>(ptr, r_pars);
}

SEXP dust_FullModelOD_rng_state(SEXP ptr, bool first_only, bool last_only) {
  return dust::r::dust_rng_state<FullModelOD>(ptr, first_only, last_only);
}

SEXP dust_FullModelOD_set_rng_state(SEXP ptr, cpp11::raws rng_state) {
  dust::r::dust_set_rng_state<FullModelOD>(ptr, rng_state);
  return R_NilValue;
}

SEXP dust_FullModelOD_set_data(SEXP ptr, cpp11::list data) {
  dust::r::dust_set_data<FullModelOD>(ptr, data);
  return R_NilValue;
}

SEXP dust_FullModelOD_compare_data(SEXP ptr, bool device) {
  return dust::r::dust_compare_data<FullModelOD>(ptr, device);
}

SEXP dust_FullModelOD_filter(SEXP ptr, bool save_trajectories,
                          cpp11::sexp step_snapshot,
                          bool device) {
  return dust::r::dust_filter<FullModelOD>(ptr, save_trajectories, step_snapshot, device);
}

cpp11::sexp dust_FullModelOD_capabilities() {
  return dust::r::dust_capabilities<FullModelOD>();
}

void dust_FullModelOD_set_n_threads(SEXP ptr, int n_threads) {
  return dust::r::dust_set_n_threads<FullModelOD>(ptr, n_threads);
}

int dust_FullModelOD_n_state(SEXP ptr) {
  return dust::r::dust_n_state<FullModelOD>(ptr);
}

cpp11::sexp dust_FullModelOD_device_info() {
  return dust::cuda::device_info<FullModelOD>();
}
