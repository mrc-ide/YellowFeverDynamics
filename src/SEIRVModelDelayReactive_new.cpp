...
// [[dust2::class(SEIRVModelDelayReactive_new)]]
...
// [[dust2::parameter(vacc_cov_cam, type = "real_type", rank = 2, required = TRUE, constant = FALSE)]]
...
// [[dust2::parameter(E_delay0, type = "real_type", rank = 2, required = TRUE, constant = FALSE)]]
// [[dust2::parameter(I_0, type = "real_type", rank = 2, required = TRUE, constant = FALSE)]]
// [[dust2::parameter(I_delay0, type = "real_type", rank = 2, required = TRUE, constant = FALSE)]]
...
class SEIRVModelDelayReactive_new {
public:
  SEIRVModelDelayReactive_new() = delete;
  ...
  struct shared_state {
    ...
    struct dim_type {
      ...
      dust2::array::dimensions<1> C_rep_total;
      dust2::array::dimensions<1> flag1;
      dust2::array::dimensions<1> flag2;
      dust2::array::dimensions<1> flag3;
      dust2::array::dimensions<1> flag4;
      dust2::array::dimensions<1> report_rate;
      dust2::array::dimensions<2> S;
      dust2::array::dimensions<2> E;
      dust2::array::dimensions<2> E_delay;
      dust2::array::dimensions<2> I;
      dust2::array::dimensions<2> I_delay;
      ...
      dust2::array::dimensions<2> vacc_rate_cam;
      dust2::array::dimensions<2> vacc_rate;
      dust2::array::dimensions<1> case_flag;
      dust2::array::dimensions<1> p_rep_cur;
      dust2::array::dimensions<1> C_rep_new;
      dust2::array::dimensions<1> F_I_total;
      dust2::array::dimensions<1> cluster_flag;
      dust2::array::dimensions<2> S_0;
      dust2::array::dimensions<2> E_0;
      dust2::array::dimensions<2> E_delay0;
      dust2::array::dimensions<2> I_0;
      dust2::array::dimensions<2> I_delay0;
      ...
      dust2::array::dimensions<2> vacc_cov_cam;
    } dim;
    real_type time_inc;
    int n_regions;
  ...
  struct internal_state {
    ...
    std::vector<real_type> case_flag;
    std::vector<real_type> p_rep_cur;
    ...
    std::vector<real_type> F_I_total;
    std::vector<real_type> E_new;
    std::vector<real_type> vacc_rate;
    std::vector<real_type> cluster_flag;
  };
  ...
  static shared_state build_shared(cpp11::list parameters) {
    ...
    const real_type inv_t_cam = 1 / t_cam;
    dim.FOI_total.set({static_cast<size_t>(n_regions)});
    dim.C_rep_total.set({static_cast<size_t>(n_regions)});
    dim.flag1.set({static_cast<size_t>(n_regions)});
    dim.flag2.set({static_cast<size_t>(n_regions)});
    dim.flag3.set({static_cast<size_t>(n_regions)});
    dim.flag4.set({static_cast<size_t>(n_regions)});
    dim.report_rate.set({static_cast<size_t>(n_regions)});
    ...
    dim.E_delay.set({static_cast<size_t>(n_regions), static_cast<size_t>(np_E_delay)});
    dim.I.set({static_cast<size_t>(n_regions), static_cast<size_t>(N_age)});
    dim.I_delay.set({static_cast<size_t>(n_regions), static_cast<size_t>(np_I_delay)});
    ...
    dim.vacc_rate_cam.set({static_cast<size_t>(n_regions), static_cast<size_t>(N_age)});
    dim.vacc_rate.set({static_cast<size_t>(n_regions), static_cast<size_t>(N_age)});
    dim.case_flag.set({static_cast<size_t>(n_regions)});
    dim.p_rep_cur.set({static_cast<size_t>(n_regions)});
    dim.C_rep_new.set({static_cast<size_t>(n_regions)});
    dim.F_I_total.set({static_cast<size_t>(n_regions)});
    dim.cluster_flag.set({static_cast<size_t>(n_regions)});
    ...
    dim.E_delay0.set({static_cast<size_t>(n_regions), static_cast<size_t>(np_E_delay)});
    dim.I_0.set({static_cast<size_t>(n_regions), static_cast<size_t>(N_age)});
    dim.I_delay0.set({static_cast<size_t>(n_regions), static_cast<size_t>(np_I_delay)});
    ...
    dim.vacc_cov_cam.set({static_cast<size_t>(n_regions), static_cast<size_t>(N_age)});
    ...
    const real_type rate4 = time_inc * inv_t_cam;
    ...
    odin.packing.state = dust2::packing{
      ...
      {"C_rep_total", std::vector<size_t>(dim.C_rep_total.dim.begin(), dim.C_rep_total.dim.end())},
      {"flag1", std::vector<size_t>(dim.flag1.dim.begin(), dim.flag1.dim.end())},
      {"flag2", std::vector<size_t>(dim.flag2.dim.begin(), dim.flag2.dim.end())},
      {"flag3", std::vector<size_t>(dim.flag3.dim.begin(), dim.flag3.dim.end())},
      {"flag4", std::vector<size_t>(dim.flag4.dim.begin(), dim.flag4.dim.end())},
      {"report_rate", std::vector<size_t>(dim.report_rate.dim.begin(), dim.report_rate.dim.end())},
    ...
    return shared_state{odin, dim, time_inc, n_regions, t_infectious, N_age, vaccine_efficacy, response_delay, case_threshold, cluster_threshold, t_cam, year0, n_years, n_t_pts, Pmin, FOI_max, np_E_delay, np_I_delay, one, p_rep, di1, di2, rate3, inv_t_cam, FOI_spillover, R0, vacc_rate_daily, vacc_cov_cam, S_0, E_0, E_delay0, I_0, I_delay0, R_0, V_0, dP1_all, dP2_all, rate4};
  }
  static internal_state build_internal(const shared_state& shared) {
    ...
    std::vector<real_type> case_flag(shared.dim.case_flag.size);
    std::vector<real_type> p_rep_cur(shared.dim.p_rep_cur.size);
    ...
    std::vector<real_type> C_rep_new(shared.dim.C_rep_new.size);
    ...
    std::vector<real_type> F_I_total(shared.dim.F_I_total.size);
    std::vector<real_type> E_new(shared.dim.E_new.size);
    std::vector<real_type> vacc_rate(shared.dim.vacc_rate.size);
    std::vector<real_type> cluster_flag(shared.dim.cluster_flag.size);
    return internal_state{I_new, R_new, P_nV, case_flag, p_rep_cur, beta, dP1, dP2, inv_P_nV, P, C_rep_new, P_tot, inv_P, FOI_sum, vacc_rate_cam, F_I_total, E_new, vacc_rate, cluster_flag};
  }
  static void update_shared(cpp11::list parameters, shared_state& shared) {
    ...
    shared.inv_t_cam = 1 / shared.t_cam;
    ...
    shared.rate4 = shared.time_inc * shared.inv_t_cam;
  }
  ...
  static void initial(real_type time, const shared_state& shared, internal_state& internal, rng_state_type& rng_state, real_type* state) {
    ...
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      state[i - 1 + shared.odin.offset.state[5]] = 0;
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
    ...
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      state[i - 1 + shared.odin.offset.state[8]] = shared.p_rep[0];
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        state[i - 1 + (j - 1) * shared.dim.S.mult[1] + shared.odin.offset.state[9]] = shared.S_0[i - 1 + (j - 1) * shared.dim.S_0.mult[1]];
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        state[i - 1 + (j - 1) * shared.dim.E.mult[1] + shared.odin.offset.state[10]] = shared.E_0[i - 1 + (j - 1) * shared.dim.E_0.mult[1]];
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.np_E_delay); ++j) {
        state[i - 1 + (j - 1) * shared.dim.E_delay.mult[1] + shared.odin.offset.state[11]] = shared.E_delay0[i - 1 + (j - 1) * shared.dim.E_delay0.mult[1]];
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        state[i - 1 + (j - 1) * shared.dim.I.mult[1] + shared.odin.offset.state[12]] = shared.I_0[i - 1 + (j - 1) * shared.dim.I_0.mult[1]];
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.np_I_delay); ++j) {
        state[i - 1 + (j - 1) * shared.dim.I_delay.mult[1] + shared.odin.offset.state[13]] = shared.I_delay0[i - 1 + (j - 1) * shared.dim.I_delay0.mult[1]];
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        state[i - 1 + (j - 1) * shared.dim.R.mult[1] + shared.odin.offset.state[14]] = shared.R_0[i - 1 + (j - 1) * shared.dim.R_0.mult[1]];
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        state[i - 1 + (j - 1) * shared.dim.V.mult[1] + shared.odin.offset.state[15]] = shared.V_0[i - 1 + (j - 1) * shared.dim.V_0.mult[1]];
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        state[i - 1 + (j - 1) * shared.dim.C.mult[1] + shared.odin.offset.state[16]] = 0;
      }
    }
  }
  static void update(real_type time, real_type dt, const real_type* state, const shared_state& shared, internal_state& internal, rng_state_type& rng_state, real_type* state_next) {
    ...
    const auto * C_rep_total = state + shared.odin.offset.state[3];
    const auto * flag1 = state + shared.odin.offset.state[4];
    const auto * flag2 = state + shared.odin.offset.state[5];
    const auto * flag3 = state + shared.odin.offset.state[6];
    const auto * flag4 = state + shared.odin.offset.state[7];
    const auto * S = state + shared.odin.offset.state[9];
    const auto * E = state + shared.odin.offset.state[10];
    const auto * E_delay = state + shared.odin.offset.state[11];
    const auto * I = state + shared.odin.offset.state[12];
    const auto * I_delay = state + shared.odin.offset.state[13];
    ...
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        internal.I_new[i - 1 + (j - 1) * shared.dim.I_new.mult[1]] = E_delay[i - 1 + (static_cast<int>(j + shared.di1) - 1) * shared.dim.E_delay.mult[1]];
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        internal.R_new[i - 1 + (j - 1) * shared.dim.R_new.mult[1]] = I_delay[i - 1 + (static_cast<int>(j + shared.di2) - 1) * shared.dim.I_delay.mult[1]];
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
    ...
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      internal.case_flag[i - 1] = (C_rep_total[i - 1] >= shared.case_threshold ? 1 : 0);
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      internal.p_rep_cur[i - 1] = (flag3[i - 1] == 1 ? shared.p_rep[1] : shared.p_rep[0]);
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
    ...
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      internal.C_rep_new[i - 1] = monty::random::binomial<real_type>(rng_state, static_cast<int>(dust2::array::sum<real_type>(internal.I_new.data(), shared.dim.I_new, {i - 1, i - 1}, {0, shared.dim.I_new.dim[1] - 1})), internal.p_rep_cur[i - 1]);
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
    ...
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        internal.vacc_rate_cam[i - 1 + (j - 1) * shared.dim.vacc_rate_cam.mult[1]] = (flag3[i - 1] == 0 ? 0 : shared.vacc_cov_cam[i - 1 + (j - 1) * shared.dim.vacc_cov_cam.mult[1]] * shared.inv_t_cam * (1 - (V[i - 1 + (j - 1) * shared.dim.V.mult[1]] * internal.inv_P[i - 1 + (j - 1) * shared.dim.inv_P.mult[1]])));
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      internal.F_I_total[i - 1] = dust2::array::sum<real_type>(I, shared.dim.I, {i - 1, i - 1}, {0, shared.dim.I.dim[1] - 1}) / internal.P_tot[i - 1];
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
    ...
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        internal.vacc_rate[i - 1 + (j - 1) * shared.dim.vacc_rate.mult[1]] = (shared.vacc_rate_daily[i - 1 + (j - 1) * shared.dim.vacc_rate_daily.mult[1] + (year_i - 1) * shared.dim.vacc_rate_daily.mult[2]] + ((flag3[i - 1] == 0 ? 0 : internal.vacc_rate_cam[i - 1 + (j - 1) * shared.dim.vacc_rate_cam.mult[1]] * monty::math::ceil<real_type>(1 - flag4[i - 1])))) * shared.vaccine_efficacy * shared.time_inc * internal.P[i - 1 + (j - 1) * shared.dim.P.mult[1]];
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      internal.cluster_flag[i - 1] = (internal.F_I_total[i - 1] >= shared.cluster_threshold ? 1 : 0);
    }
    ...
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      state_next[i - 1 + shared.odin.offset.state[3]] = C_rep_total[i - 1] + internal.C_rep_new[i - 1];
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      state_next[i - 1 + shared.odin.offset.state[4]] = monty::math::min<real_type>(shared.one, flag1[i - 1] + (internal.case_flag[i - 1] * shared.rate3));
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      state_next[i - 1 + shared.odin.offset.state[5]] = monty::math::min<real_type>(shared.one, flag2[i - 1] + (internal.cluster_flag[i - 1] * shared.rate3));
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      state_next[i - 1 + shared.odin.offset.state[6]] = (flag1[i - 1] == 1 ? 1 : ((flag2[i - 1] == 1 ? 1 : 0)));
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      state_next[i - 1 + shared.odin.offset.state[7]] = (flag3[i - 1] == 0 ? 0 : monty::math::min<real_type>(shared.one, flag4[i - 1] + shared.rate4));
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      state_next[i - 1 + shared.odin.offset.state[8]] = internal.p_rep_cur[i - 1];
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      state_next[i - 1 + shared.odin.offset.state[9]] = monty::math::max<real_type>(shared.Pmin, S[i - 1] - internal.E_new[i - 1] - internal.vacc_rate[i - 1] * S[i - 1] * internal.inv_P_nV[i - 1] + internal.dP1[i - 1] - (internal.dP2[i - 1] * S[i - 1] * internal.inv_P[i - 1]));
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 2; j <= static_cast<size_t>(shared.N_age); ++j) {
        state_next[i - 1 + (j - 1) * shared.dim.S.mult[1] + shared.odin.offset.state[9]] = monty::math::max<real_type>(shared.Pmin, S[i - 1 + (j - 1) * shared.dim.S.mult[1]] - internal.E_new[i - 1 + (j - 1) * shared.dim.E_new.mult[1]] - internal.vacc_rate[i - 1 + (j - 1) * shared.dim.vacc_rate.mult[1]] * S[i - 1 + (j - 1) * shared.dim.S.mult[1]] * internal.inv_P_nV[i - 1 + (j - 1) * shared.dim.inv_P_nV.mult[1]] + (internal.dP1[i - 1 + (j - 1) * shared.dim.dP1.mult[1]] * S[i - 1 + (j - 1 - 1) * shared.dim.S.mult[1]] * internal.inv_P[i - 1 + (j - 1 - 1) * shared.dim.inv_P.mult[1]]) - (internal.dP2[i - 1 + (j - 1) * shared.dim.dP2.mult[1]] * S[i - 1 + (j - 1) * shared.dim.S.mult[1]] * internal.inv_P[i - 1 + (j - 1) * shared.dim.inv_P.mult[1]]));
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        state_next[i - 1 + (j - 1) * shared.dim.E.mult[1] + shared.odin.offset.state[10]] = monty::math::max<real_type>(shared.Pmin, E[i - 1 + (j - 1) * shared.dim.E.mult[1]] + internal.E_new[i - 1 + (j - 1) * shared.dim.E_new.mult[1]] - internal.I_new[i - 1 + (j - 1) * shared.dim.I_new.mult[1]]);
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = (shared.N_age + 1); j <= static_cast<size_t>(shared.np_E_delay); ++j) {
        state_next[i - 1 + (j - 1) * shared.dim.E_delay.mult[1] + shared.odin.offset.state[11]] = E_delay[i - 1 + (j - shared.N_age - 1) * shared.dim.E_delay.mult[1]];
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        state_next[i - 1 + (j - 1) * shared.dim.E_delay.mult[1] + shared.odin.offset.state[11]] = internal.E_new[i - 1 + (j - 1) * shared.dim.E_new.mult[1]];
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        state_next[i - 1 + (j - 1) * shared.dim.I.mult[1] + shared.odin.offset.state[12]] = monty::math::max<real_type>(shared.Pmin, I[i - 1 + (j - 1) * shared.dim.I.mult[1]] + internal.I_new[i - 1 + (j - 1) * shared.dim.I_new.mult[1]] - internal.R_new[i - 1 + (j - 1) * shared.dim.R_new.mult[1]]);
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = (shared.N_age + 1); j <= static_cast<size_t>(shared.np_I_delay); ++j) {
        state_next[i - 1 + (j - 1) * shared.dim.I_delay.mult[1] + shared.odin.offset.state[13]] = I_delay[i - 1 + (j - shared.N_age - 1) * shared.dim.I_delay.mult[1]];
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        state_next[i - 1 + (j - 1) * shared.dim.I_delay.mult[1] + shared.odin.offset.state[13]] = internal.I_new[i - 1 + (j - 1) * shared.dim.I_new.mult[1]];
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      state_next[i - 1 + shared.odin.offset.state[14]] = monty::math::max<real_type>(shared.Pmin, R[i - 1] + internal.R_new[i - 1] - internal.vacc_rate[i - 1] * R[i - 1] * internal.inv_P_nV[i - 1] - (internal.dP2[i - 1] * R[i - 1] * internal.inv_P[i - 1]));
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 2; j <= static_cast<size_t>(shared.N_age); ++j) {
        state_next[i - 1 + (j - 1) * shared.dim.R.mult[1] + shared.odin.offset.state[14]] = monty::math::max<real_type>(shared.Pmin, R[i - 1 + (j - 1) * shared.dim.R.mult[1]] + internal.R_new[i - 1 + (j - 1) * shared.dim.R_new.mult[1]] - internal.vacc_rate[i - 1 + (j - 1) * shared.dim.vacc_rate.mult[1]] * R[i - 1 + (j - 1) * shared.dim.R.mult[1]] * internal.inv_P_nV[i - 1 + (j - 1) * shared.dim.inv_P_nV.mult[1]] + (internal.dP1[i - 1 + (j - 1) * shared.dim.dP1.mult[1]] * R[i - 1 + (j - 1 - 1) * shared.dim.R.mult[1]] * internal.inv_P[i - 1 + (j - 1 - 1) * shared.dim.inv_P.mult[1]]) - (internal.dP2[i - 1 + (j - 1) * shared.dim.dP2.mult[1]] * R[i - 1 + (j - 1) * shared.dim.R.mult[1]] * internal.inv_P[i - 1 + (j - 1) * shared.dim.inv_P.mult[1]]));
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      state_next[i - 1 + shared.odin.offset.state[15]] = monty::math::max<real_type>(shared.Pmin, V[i - 1] + internal.vacc_rate[i - 1] - (internal.dP2[i - 1] * V[i - 1] * internal.inv_P[i - 1]));
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 2; j <= static_cast<size_t>(shared.N_age); ++j) {
        state_next[i - 1 + (j - 1) * shared.dim.V.mult[1] + shared.odin.offset.state[15]] = monty::math::max<real_type>(shared.Pmin, V[i - 1 + (j - 1) * shared.dim.V.mult[1]] + internal.vacc_rate[i - 1 + (j - 1) * shared.dim.vacc_rate.mult[1]] + (internal.dP1[i - 1 + (j - 1) * shared.dim.dP1.mult[1]] * V[i - 1 + (j - 1 - 1) * shared.dim.V.mult[1]] * internal.inv_P[i - 1 + (j - 1 - 1) * shared.dim.inv_P.mult[1]]) - (internal.dP2[i - 1 + (j - 1) * shared.dim.dP2.mult[1]] * V[i - 1 + (j - 1) * shared.dim.V.mult[1]] * internal.inv_P[i - 1 + (j - 1) * shared.dim.inv_P.mult[1]]));
      }
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.n_regions); ++i) {
      for (size_t j = 1; j <= static_cast<size_t>(shared.N_age); ++j) {
        state_next[i - 1 + (j - 1) * shared.dim.C.mult[1] + shared.odin.offset.state[16]] = internal.I_new[i - 1 + (j - 1) * shared.dim.I_new.mult[1]];
      }
    }
  }
};
...
SEXP dust2_system_SEIRVModelDelayReactive_new_alloc(cpp11::list r_pars, cpp11::sexp r_time, cpp11::list r_time_control, cpp11::sexp r_n_particles, cpp11::sexp r_n_groups, cpp11::sexp r_seed, cpp11::sexp r_deterministic, cpp11::sexp r_n_threads) {
  return dust2::r::dust2_discrete_alloc<SEIRVModelDelayReactive_new>(r_pars, r_time, r_time_control, r_n_particles, r_n_groups, r_seed, r_deterministic, r_n_threads);
}
[[cpp11::register]]
SEXP dust2_system_SEIRVModelDelayReactive_new_run_to_time(cpp11::sexp ptr, cpp11::sexp r_time) {
  return dust2::r::dust2_system_run_to_time<dust2::dust_discrete<SEIRVModelDelayReactive_new>>(ptr, r_time);
}
...
SEXP dust2_system_SEIRVModelDelayReactive_new_state(cpp11::sexp ptr, cpp11::sexp r_index_state, cpp11::sexp r_index_particle, cpp11::sexp r_index_group, bool preserve_particle_dimension, bool preserve_group_dimension) {
  return dust2::r::dust2_system_state<dust2::dust_discrete<SEIRVModelDelayReactive_new>>(ptr, r_index_state, r_index_particle, r_index_group, preserve_particle_dimension, preserve_group_dimension);
}

[[cpp11::register]]
SEXP dust2_system_SEIRVModelDelayReactive_new_time(cpp11::sexp ptr) {
  return dust2::r::dust2_system_time<dust2::dust_discrete<SEIRVModelDelayReactive_new>>(ptr);
}

[[cpp11::register]]
SEXP dust2_system_SEIRVModelDelayReactive_new_set_state_initial(cpp11::sexp ptr) {
  return dust2::r::dust2_system_set_state_initial<dust2::dust_discrete<SEIRVModelDelayReactive_new>>(ptr);
}

[[cpp11::register]]
SEXP dust2_system_SEIRVModelDelayReactive_new_set_state(cpp11::sexp ptr, cpp11::list r_state) {
  return dust2::r::dust2_system_set_state<dust2::dust_discrete<SEIRVModelDelayReactive_new>>(ptr, r_state);
}

[[cpp11::register]]
SEXP dust2_system_SEIRVModelDelayReactive_new_reorder(cpp11::sexp ptr, cpp11::integers r_index) {
  return dust2::r::dust2_system_reorder<dust2::dust_discrete<SEIRVModelDelayReactive_new>>(ptr, r_index);
}

[[cpp11::register]]
SEXP dust2_system_SEIRVModelDelayReactive_new_rng_state(cpp11::sexp ptr) {
  return dust2::r::dust2_system_rng_state<dust2::dust_discrete<SEIRVModelDelayReactive_new>>(ptr);
}

[[cpp11::register]]
SEXP dust2_system_SEIRVModelDelayReactive_new_set_rng_state(cpp11::sexp ptr, cpp11::sexp r_rng_state) {
  return dust2::r::dust2_system_set_rng_state<dust2::dust_discrete<SEIRVModelDelayReactive_new>>(ptr, r_rng_state);
}

[[cpp11::register]]
SEXP dust2_system_SEIRVModelDelayReactive_new_set_time(cpp11::sexp ptr, cpp11::sexp r_time) {
  return dust2::r::dust2_system_set_time<dust2::dust_discrete<SEIRVModelDelayReactive_new>>(ptr, r_time);
}

[[cpp11::register]]
SEXP dust2_system_SEIRVModelDelayReactive_new_update_pars(cpp11::sexp ptr, cpp11::list pars) {
  return dust2::r::dust2_system_update_pars<dust2::dust_discrete<SEIRVModelDelayReactive_new>>(ptr, pars);
}
...
SEXP dust2_system_SEIRVModelDelayReactive_new_simulate(cpp11::sexp ptr, cpp11::sexp r_times, cpp11::sexp r_index_state, bool preserve_particle_dimension, bool preserve_group_dimension) {
  return dust2::r::dust2_system_simulate<dust2::dust_discrete<SEIRVModelDelayReactive_new>>(ptr, r_times, r_index_state, preserve_particle_dimension, preserve_group_dimension);
}
