...
[[cpp11::register]]
SEXP dust_cpu_FullModelODVarFR_alloc(cpp11::list r_pars, bool pars_multi, cpp11::sexp r_time,
...
[[cpp11::register]]
SEXP dust_cpu_FullModelODVarFR_update_state(SEXP ptr, SEXP r_pars, SEXP r_state,
...
[[cpp11::register]]
SEXP dust_cpu_FullModelODVarFR_filter(SEXP ptr, SEXP time_end,
...
public:
  ...
  FullModelODVarFR(const dust::pars_type<FullModelODVarFR>& pars) :
  ...
  void update(size_t step, const real_type * state, rng_state_type& rng_state, real_type * state_next) {
    ...
    real_type FOI_sum = std::min(shared->FOI_max, beta * (odin_sum1<real_type>(I, 0, shared->dim_I) / (real_type) P_tot) + (shared->FOI_spillover[static_cast<int>(year_i) - 1] * shared->dt));
    {
       int i = 1;
...
namespace dust {
template<>
dust::pars_type<FullModelODVarFR> dust_pars<FullModelODVarFR>(cpp11::list user) {
  using real_type = typename FullModelODVarFR::real_type;
  auto shared = std::make_shared<FullModelODVarFR::shared_type>();
  FullModelODVarFR::internal_type internal;
  shared->FOI_max = 1;
  ...
  shared->dim_FOI_spillover = shared->n_years;
  ...
  return dust::pars_type<FullModelODVarFR>(shared, internal);
}
template <>
cpp11::sexp dust_info<FullModelODVarFR>(const dust::pars_type<FullModelODVarFR>& pars) {
  const FullModelODVarFR::internal_type internal = pars.internal;
  const std::shared_ptr<const FullModelODVarFR::shared_type> shared = pars.shared;
...
cpp11::sexp dust_FullModelODVarFR_capabilities() {
  return dust::r::dust_capabilities<FullModelODVarFR>();
}

cpp11::sexp dust_FullModelODVarFR_gpu_info() {
  return dust::gpu::r::gpu_info();
}
using model_cpu = dust::dust_cpu<FullModelODVarFR>;

SEXP dust_cpu_FullModelODVarFR_alloc(cpp11::list r_pars, bool pars_multi, cpp11::sexp r_time,
                             cpp11::sexp r_n_particles, int n_threads,
                             cpp11::sexp r_seed, bool deterministic,
                             cpp11::sexp gpu_config, cpp11::sexp ode_control) {
  return dust::r::dust_cpu_alloc<FullModelODVarFR>(r_pars, pars_multi, r_time, r_n_particles,
...
SEXP dust_cpu_FullModelODVarFR_simulate(SEXP ptr, cpp11::sexp r_time_end) {
...
SEXP dust_cpu_FullModelODVarFR_set_data(SEXP ptr, cpp11::list data,
                                       bool shared) {
