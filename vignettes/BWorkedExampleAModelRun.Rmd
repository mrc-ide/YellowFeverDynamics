...
mode_start <- 1       
# Number of particles to run (maximum allowed value = 20)
n_particles <- 10     
# Number of threads to run (equal to or less than n_particles)
n_threads <- 1   
# Year at which to stop generating data (year after last year where data is desired)
year_end <- 2001     
# Year to begin saving data
...
```{r,eval=FALSE}
test1 <- Full_Model_Run(FOI_spillover, R0, vacc_data, pop_data, year0, 
...
```{r,eval = FALSE}
test2 <- Basic_Model_Run(FOI_spillover, R0, vacc_data, pop_data, year0, 
                         mode_start, n_particles, n_threads, year_end,
                         year_data_begin, vaccine_efficacy, start_SEIRV, dt)
```
...
R0 <- 0.0
test3 <- case_data_generate(FOI_spillover, R0, vacc_data, pop_data, year0, 
                            mode_start, n_reps, year_end,
                            year_data_begin, vaccine_efficacy, start_SEIRV, dt)
```
