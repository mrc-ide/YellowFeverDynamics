...
F_I_total[1:n_regions] <- sum(I[i,])/P_tot[i] #Total no. currently infectious people as fraction of population - check for cluster flag
cluster_flag[1:n_regions] <- if(F_I_total[i]>=cluster_threshold) 1 else 0
...
update(C_rep_total[1:n_regions]) <- C_rep_total[i] + C_rep_new[i] #Running total reported cases across all ages
update(flag1[1:n_regions]) <- min(one,flag1[i] + (case_flag[i]*rate3)) #case flag with delay (converted to integer on use)
update(flag2[1:n_regions]) <- min(one,flag2[i] + (cluster_flag[i]*rate3)) #cluster flag with delay (converted to integer on use)
update(flag3[1:n_regions]) <- if(flag1[i]==1) 1 else (if(flag2[i]==1) 1 else 0) #0 = No emergency (flags 1+2 not tripped); 1 = emergency (flag 1 and/or 2 tripped)
update(flag4[1:n_regions]) <- if(flag3[i]==0) 0 else min(one,flag4[i]+rate4)
update(report_rate[1:n_regions]) <- p_rep_cur[i]
...
initial(C_rep_total[1:n_regions]) <- 0
initial(flag1[1:n_regions]) <- 0
initial(flag2[1:n_regions]) <- 0
initial(flag3[1:n_regions]) <- 0
initial(flag4[1:n_regions]) <- 0
initial(report_rate[1:n_regions]) <- p_rep[1]
...
dim(C_rep_total) <- n_regions
dim(flag1) <- n_regions
dim(flag2) <- n_regions
dim(flag3) <- n_regions
dim(flag4) <- n_regions
dim(report_rate) <- n_regions
...
dim(vacc_rate_cam) <- c(n_regions, N_age)
dim(vacc_rate) <- c(n_regions, N_age)
dim(case_flag) <- n_regions
dim(p_rep_cur) <- n_regions
dim(C_rep_new) <- n_regions
...
dim(p_rep) <- 2
dim(vacc_cov_cam) <- c(n_regions, N_age)
