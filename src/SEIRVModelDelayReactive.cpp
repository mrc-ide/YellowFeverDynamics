...
public:
  ...
  static dust2::packing packing_state(const shared_state& shared) {
    return dust2::packing{
      ...
      {"E_delay", std::vector<size_t>(shared.dim.E_delay.dim.begin(), shared.dim.E_delay.dim.end())},
      {"I", std::vector<size_t>(shared.dim.I.dim.begin(), shared.dim.I.dim.end())},
      {"I_delay", std::vector<size_t>(shared.dim.I_delay.dim.begin(), shared.dim.I_delay.dim.end())},
  ...
  static void initial(real_type time, const shared_state& shared, internal_state& internal, rng_state_type& rng_state, real_type* state) {
    ...
    for (size_t i = 1; i <= static_cast<size_t>(shared.np_E_delay); ++i) {
      state[i - 1 + shared.offset.state.E_delay] = shared.E_delay0[i - 1];
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.N_age); ++i) {
      state[i - 1 + shared.offset.state.I] = shared.I_0[i - 1];
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.np_I_delay); ++i) {
      state[i - 1 + shared.offset.state.I_delay] = shared.I_delay0[i - 1];
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.N_age); ++i) {
  ...
  static void update(real_type time, real_type dt, const real_type* state, const shared_state& shared, internal_state& internal, rng_state_type& rng_state, real_type* state_next) {
    ...
    const auto * E_delay = state + shared.offset.state.E_delay;
    const auto * I = state + shared.offset.state.I;
    const auto * I_delay = state + shared.offset.state.I_delay;
    ...
    for (size_t i = (shared.N_age + 1); i <= static_cast<size_t>(shared.np_E_delay); ++i) {
      state_next[i - 1 + shared.offset.state.E_delay] = E_delay[static_cast<int>(i) - shared.N_age - 1];
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.N_age); ++i) {
      state_next[i - 1 + shared.offset.state.E_delay] = internal.E_new[i - 1];
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.N_age); ++i) {
    ...
    for (size_t i = (shared.N_age + 1); i <= static_cast<size_t>(shared.np_I_delay); ++i) {
      state_next[i - 1 + shared.offset.state.I_delay] = I_delay[static_cast<int>(i) - shared.N_age - 1];
    }
    for (size_t i = 1; i <= static_cast<size_t>(shared.N_age); ++i) {
      state_next[i - 1 + shared.offset.state.I_delay] = internal.I_new[i - 1];
    }
