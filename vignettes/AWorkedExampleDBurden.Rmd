...
This vignette demonstrates how to use the package to calculate annual yellow fever burden () for multiple regions. The total_burden_estimate() function is used for this, using similar inputs to the data_match_single() and data_match_multi() functions described in [Worked Example 3 - Generating Epidemiological Data Sets](https://mrc-ide.github.io/YellowFeverDynamics/articles/AWorkedExampleCDataGenerate.html):
...
param_dist <- exp(read.csv(file = paste(path.package("YellowFeverDynamics"), "/exdata/enviro_coeffs_example2.csv", sep = ""), header = TRUE)) # Data frame containing one or more sets of parameter values (here the actual values are used, rather than the logarithmic values as in the case of the data-matching functions)
...
vaccine_efficacy0 <- 1.0 # Vaccine efficacy
p_rep_severe0 <- 0.1 # Probability of reporting of individuals with severe symptoms
p_rep_death0 <- 0.2 # Probability of reporting of fatal infections
flag_reporting=TRUE #Flag indicating whether to calculate numbers of reported cases and deaths (using p_rep_severe0 and p_rep_death0) in addition to actual numbers
results<-total_burden_estimate(type,param_dist,input_data,NULL,years_data,n_reps,mode_start,dt,enviro_data,
                               R0_fixed_values,vaccine_efficacy0,p_rep_severe0,p_rep_death0,flag_reporting)
```
...
The output of total_burden_estimate() may be used, for example, to model vaccine impact by examining the difference in burden between a no-vaccination scenario and a scenario featuring a specified vaccination program.
